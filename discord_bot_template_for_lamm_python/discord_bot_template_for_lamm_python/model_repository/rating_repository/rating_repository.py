...
class RatingRepository(Generic[T,Y], BaseModelRepository[T,Y]):
    ...
    def _get_base_model_from_map_and_list_keys(self, map: dict[str, object], list_keys: list[str]) -> T:
        ...
        if map.get(list_keys[0]) is None:
            return Rating("",0)
        if len(list_keys) <= 1:
            return Rating(list_keys[0],0)
        if map.get(list_keys[1]) is None:
            return Rating(list_keys[0],0)
        return Rating(map.get(list_keys[0]),map.get(list_keys[1]))
    ...
    async def get_rating_from_unique_id_by_user_parameter_one(self, unique_id_by_user: str) -> Result:
        return await self._get_mode_callback_from_release_callback_and_test_callback_parameter_enum_rwt_mode(
            self.__get_rating_from_unique_id_by_user_parameter_one_w_release_callback,
            self.__get_rating_from_unique_id_by_user_parameter_one_w_test_callback)(unique_id_by_user)
    
    async def __get_rating_from_unique_id_by_user_parameter_one_w_release_callback(self, unique_id_by_user: str) -> Result:
    ...
    async def __get_rating_from_unique_id_by_user_parameter_one_w_test_callback(self, unique_id_by_user: str) -> Result:
